...
import kotlinx.android.synthetic.main.activity_ks4_bs.*
import org.w3c.dom.Text
import java.lang.Exception

class Ks4BsActivity : AppCompatActivity() {

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_ks4_bs)
    ...
    fun btnhesabla4(view: View){
        try {
            if ((!TextUtils.isEmpty(this.kqs1.getText().toString())) && (!TextUtils.isEmpty(this.kqs2.getText().toString())) &&
                ...
                && (!TextUtils.isEmpty(this.bqs.getText().toString()))){
                ...
                if ((ksq1 <= 100.0F) && (ksq1 >= 0.0F) && (ksq2 <=100.0F) && (ksq2 >= 0.0F) && (ksq3 <= 100.0F) && (ksq3 >= 0.0F) &&
                        (ksq4 <= 100.0F) && (ksq4 >= 0.0F) && (bsq <= 100.0F) && (bsq >= 0.0F)){
                    var cvb1 : Float = (((ksq1 + ksq2 + ksq3 + ksq4) / 4 * 0.4F) + (bsq * 0.6F))
